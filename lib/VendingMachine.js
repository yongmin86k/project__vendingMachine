...
const _resupplyCoins = async (inventory, coinTypes, resupplyCoins) => {
  return new Promise(resolve => {
    Object.keys(resupplyCoins).reduce((_, cur) => {
      inventory.hasCoins[cur] += resupplyCoins[cur];
    }, 0);
    const result = _currentBalance(inventory, coinTypes);
...
class _VendingMachine {
  ...
  async resupplyCoins(resupplyCoins) {
    const result = await _resupplyCoins(
      this.inventory,
      this.coinTypes,
      resupplyCoins
    );
